    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            <div className="relative w-full max-w-sm rounded-2xl bg-navy-900 border border-border shadow-elevated p-6 text-center animate-scale-in">
                <div
                    className={`mx-auto w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${variant === 'danger' ? 'bg-red-500/10' : 'bg-green-500/10'}`}
                >
                    {variant === 'danger' ? (
...
function AgentRequestRow({
    ...
    const typeCls =
        CHAIN_TYPE_COLOR[req.chainType] || 'bg-slate-500/10 text-slate-400';
    ...
    return (
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border/60 last:border-0 hover:bg-surface-hover transition-colors">
            ...
            <div className="hidden sm:flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-navy-950 border border-border text-xs">
                    ...
                    <span className="text-white font-medium">
                        {req.networkChainId ?? `#${req.chainId}`}
                    </span>
                </div>
                {req.chainType && (
                    <span
                        className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${typeCls}`}
                    >
                        {typeLabel}
                    </span>
                )}
                {req.chainStatus && (
                    <Badge
                        variant={
                            req.chainStatus === 'completed'
                                ? 'success'
                                : req.chainStatus === 'broken'
                                  ? 'danger'
                                  : 'outline'
                        }
                        className="text-[11px]"
            ...
            {/* Timestamp */}
            <p className="hidden md:block text-[11px] text-muted-foreground shrink-0 w-16 text-right">
            ...
            <div className="relative w-full max-w-sm rounded-2xl bg-navy-900 border border-border shadow-elevated animate-scale-in">
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                    ...
                    <p className="text-xs text-muted">
                        An invitation to join Kovera will be sent to this
                        address. The agent will be associated with this chain
                        node upon confirmation.
                    </p>
                    ...
                    <div className="flex items-center gap-3 pt-1">
                        ...
                        >
                            {loading ? (
                                ...
                                <>
                                    <Mail className="w-3.5 h-3.5" /> Assign &
                                    Invite
                                </>
                            )}
                        </Button>
...
const ROLE_ACCENT = {
    ...
    landlord: {
        ...
        label: 'bg-amber-500/10 text-amber-400 border border-amber-500/20',
    },
    renter: {
        bar: '#a855f7',
        barBg: 'rgba(168,85,247,0.15)',
        avatar: 'bg-purple-500/20 text-purple-300',
        label: 'bg-purple-500/10 text-purple-400 border border-purple-500/20',
    },
};
const DEFAULT_ACCENT = {
    ...
    label: 'bg-slate-500/10 text-slate-400 border border-slate-500/20',
};
...
function FlowNode({ participant, index, onAssign }) {
    ...
    return (
        <div
            className={`relative flex flex-col w-52 shrink-0 rounded-2xl border bg-navy-900 overflow-hidden transition-all duration-200 group ${outerCls}`}
        >
            {/* Header band */}
            <div
            ...
            >
                <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 ${accent.avatar} ring-1 ring-white/5`}
                >
                    {(participant.name || participant.userId || '?')
                        .charAt(0)
                        .toUpperCase()}
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                        <span
                            className={`px-1.5 py-px rounded text-[9px] font-semibold uppercase tracking-wide ${accent.label}`}
                        >
            ...
            <div className="px-3.5 pb-3 pt-2 flex flex-col gap-2">
                ...
                {agented &&
                (participant.agentName || participant.agentEmail) ? (
                    <div className="flex items-start gap-2 px-2.5 py-2 rounded-xl bg-green-500/8 border border-green-500/20">
                        ...
                        <div className="min-w-0">
                            {participant.agentName && (
                                <p className="text-[10px] font-semibold text-green-300 leading-tight truncate">
                                    {participant.agentName}
                                </p>
                            )}
                    ...
                    <div className="flex items-start gap-2 px-2.5 py-2 rounded-xl bg-blue-500/8 border border-blue-500/20">
                        ...
                        <div className="min-w-0">
                            <p className="text-[10px] font-semibold text-blue-300 leading-tight">
                                Pending review
                            </p>
                            <p className="text-[10px] text-blue-400/70 truncate">
                                {participant.pendingAgentEmail}
                            </p>
                        </div>
                    </div>
                ) : (
...
function ChainRow({ chain, onAssignNode }) {
    ...
    const typeCls =
        CHAIN_TYPE_COLOR[chain.chainType] || 'bg-slate-500/10 text-slate-400';
    const typeLabel =
        CHAIN_TYPE_LABEL[chain.chainType] || chain.chainType || 'Unknown';
    const hasPending = pendingCount > 0;
    const pct =
        participants.length > 0
            ? (agentedCount / participants.length) * 100
            : 0;
    ...
    return (
        <div
            className={`rounded-2xl border bg-navy-950 overflow-hidden transition-all duration-200 ${expanded ? 'border-border-light shadow-md' : hasPending ? 'border-blue-500/30' : fullyAgented ? 'border-green-500/20' : 'border-border hover:border-border-light'}`}
        >
            <div className="flex">
                <div className="flex-1 min-w-0">
                    ...
                    >
                        {/* Chevron */}
                        <div
                            className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 transition-colors ${expanded ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-white'}`}
                        >
                            {expanded ? (
                        ...
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                            <div className="flex items-center gap-2 min-w-0">
                                ...
                                <span
                                    className={`px-2 py-0.5 rounded-lg text-[11px] font-medium ${typeCls}`}
                                >
                                    {typeLabel}
                                </span>
                            </div>
                            ...
                            <div className="hidden sm:flex flex-col items-end gap-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[11px] text-muted w-8 text-right">
                                        {agentedCount}/{participants.length}
                                    </span>
                                </div>
                        ...
                        <div className="border-t border-border/50">
                            ...
                            <div className="flex items-center gap-4 px-5 py-2.5 border-t border-border/40 bg-navy-900/40">
                                <div className="flex items-center gap-3 flex-wrap">
                                    {Object.entries(ROLE_ACCENT).map(
                                        ([role, a]) => (
                                            <span
                                                key={role}
...
export default function ChainManagementPage() {
    ...
    const activeRequests = useMemo(
        () => allRequests.filter((r) => r.chainStatus !== 'broken'),
        [allRequests],
    );
    const brokenChainRequests = useMemo(
        () => allRequests.filter((r) => r.chainStatus === 'broken'),
        [allRequests],
    );
    ...
    useEffect(() => {
        const toClose = brokenChainRequests.filter(
            (r) => !closedIdsRef.current.has(r.id),
        );
        if (!toClose.length) return;
        toClose.forEach((r) => closedIdsRef.current.add(r.id));
        Promise.all(
            toClose.map((r) =>
                analyticsAPI.rejectAgentRequest(r.id).catch(() => {}),
            ),
        ).then(() =>
    ...
    const needsAgentCount = chains.filter((c) =>
        c.participants?.some((p) => !p.hasAgent),
    ).length;
    const fullyAgented = chains.filter((c) =>
        c.participants?.every((p) => p.hasAgent),
    ).length;
    ...
    const filteredChains = useMemo(() => {
        return chains.filter((c) => {
            if (chainSearch) {
                ...
                const matchType = (
                    CHAIN_TYPE_LABEL[c.chainType] ||
                    c.chainType ||
                    ''
                )
                    .toLowerCase()
                    .includes(q);
                const matchName = c.participants?.some((p) =>
                    p.name?.toLowerCase().includes(q),
                );
                if (!matchId && !matchType && !matchName) return false;
            }
            if (typeFilter !== 'all' && c.chainType !== typeFilter)
                return false;
            if (agentFilter === 'needs-agent')
                return c.participants?.some((p) => !p.hasAgent);
            if (agentFilter === 'fully-agented')
                return c.participants?.every((p) => p.hasAgent);
            return true;
        });
    ...
    const reqTotalPages = Math.max(
        1,
        Math.ceil(requests.length / REQS_PER_PAGE),
    );
    const chainTotalPages = Math.max(
        1,
        Math.ceil(filteredChains.length / CHAINS_PER_PAGE),
    );

    const pagedRequests = requests.slice(
        (reqPage - 1) * REQS_PER_PAGE,
        reqPage * REQS_PER_PAGE,
    );
    const pagedChains = filteredChains.slice(
        (chainPage - 1) * CHAINS_PER_PAGE,
        chainPage * CHAINS_PER_PAGE,
    );

    /* ── Reset chain page when filters change ── */
    const handleChainSearch = (v) => {
        setChainSearch(v);
        setChainPage(1);
    };
    ...
    const chainTypes = useMemo(() => {
        const types = [
            ...new Set(chains.map((c) => c.chainType).filter(Boolean)),
        ];
        return types;
    }, [chains]);
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted mt-1 text-sm">
                        Review agent association requests and manage chain
                        participants.
                    </p>
                </div>
                    ...
                    {requestsQ.isLoading ? (
                        ...
                        <>
                            ...
                            <div className="flex items-center gap-3 px-4 py-2 border-b border-border">
                                ...
                                <p className="flex-1 text-xs font-medium text-muted uppercase tracking-wider">
                                    User / Agent
                                </p>
                                <p className="hidden sm:block text-xs font-medium text-muted uppercase tracking-wider w-48">
                                    Chain
                                </p>
                                <p className="hidden md:block text-xs font-medium text-muted uppercase tracking-wider w-16 text-right">
                                    Time
                                </p>
                                <p className="text-xs font-medium text-muted uppercase tracking-wider w-36 text-right">
                                    Actions
                                </p>
                            </div>
                            <div>
                                {pagedRequests.map((req) => (
                                    <AgentRequestRow
                                        ...
                                        inviting={
                                            invite.isPending &&
                                            invite.variables === req.id
                                        }
                                        approving={
                                            approve.isPending &&
                                            approve.variables === req.id
                                        }
                                        rejecting={
                                            reject.isPending &&
                                            reject.variables === req.id
                                        }
                                    />
                                ))}
                            </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                        ...
                        <div className="relative flex-1 max-w-sm w-full">
                            ...
                            <input
                                ...
                                value={chainSearch}
                                onChange={(e) =>
                                    handleChainSearch(e.target.value)
                                }
